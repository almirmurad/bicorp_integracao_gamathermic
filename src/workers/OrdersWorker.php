...
// print_r(__DIR__);
// require_once '/home/u287435636/domains/bicorp.online/public_html/gamatermic/vendor/autoload.php';
// require_once '/home/u287435636/domains/bicorp.online/public_html/gamatermic/src/services/RabbitMQServices.php';
...
// $dotenv = Dotenv::createUnsafeImmutable('/home/u287435636/domains/bicorp.online/public_html/gamatermic/', '.env');
$dotenv = Dotenv::createUnsafeImmutable('./', '.env');
$dotenv->load();

// Conectar ao RabbitMQ
$connection = new AMQPStreamConnection($_ENV['IP'], $_ENV['PORT'], $_ENV['RABBITMQ_USER'], $_ENV['RABBITMQ_PASS'], $_ENV['RABBITMQ_VHOST']);
...
$trash_queue_name = 'ploomes_orders_trash';
    ...
    $channel->queue_declare($queue_name, false, true, false, false, false, [   // Argumentos para a DLX
        'x-dead-letter-exchange' =>  ['S', 'orders_exc_wait'],  // Define a Dead Letter Exchange
        'x-dead-letter-routing-key' => ['S', 'Ploomes.Orders.Wait'] // Define a Routing Key para a DLX
    ]);
    $channel->queue_declare('ploomes_orders_trash', false, true, false, false, false);
...
//             if ($retryCount >= 3) {
//                 // Manda para fila de erro definitiva ou log orders_exc_trash

//                 // Publica a mensagem na fila
//                 $this->channel->basic_publish($msg, 'orders_exc_trash');
...
$callback = function($msg) use ($channel,$trash_binding_key) {
    try {
        ...
        $uri = 'http://localhost/gamatermic/public/processNewOrder';
