...
Class PloomesFormatter implements CrmFormattersInterface{
            ...
            foreach($contact->basesFaturamento as $k => $bf)
            {
                $omie[$k] = new stdClass();
                    if(isset($bf['integrar']) && $bf['integrar']  > 0){
                        ...
                        $contact->cVendedorOmie = (isset($contact->ownerEmail) && $contact->ownerEmail !== null) ? $this->nasajonServices->vendedorIdOmie($omie[$k],$contact->ownerEmail) : null;
                        $contact->cTransportadora = $contact->cTranspOmie[$k] ?? null;
                        // $alterar = $this->nasajonServices->alteraCliente($omie[$k], $diff);
                      
                        $alterar = $this->nasajonServices->alteraClienteCRMToERP($omie[$k], $contact);
    ...
    public function createContact(object $contact, object $ploomesServices):array
    {
        ...
        foreach($contact->basesFaturamento as $k => $bf)
        {
            ...
            if(isset($bf['integrar']) &&  $bf['integrar'] > 0){
                ...
                $omie[$k]->appSecret = $bf['app_secret'];
                $omie[$k]->appKey = $bf['app_key'];
                $contact->cVendedorOmie = $this->nasajonServices->vendedorIdOmie($omie[$k],$contact->ownerEmail) ?? null; 
                $contact->idTransportadora = $this->nasajonServices->getShipping($omie[$k], $contact);
                $criaClienteOmie = $this->nasajonServices->criaClienteOmie($omie[$k], $contact);
