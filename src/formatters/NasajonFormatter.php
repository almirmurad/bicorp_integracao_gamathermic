...
Class NasajonFormatter implements ErpFormattersInterface{
    ...
    {
        $this->nasajonServices = new NasajonServices($appk, $omieBases);
    }
    ...
    public function createClientErpToCrmObj(array $args):object
    {
        ...
        $omieApp = $this->nasajonServices->getOmieApp();

        $c =  $this->nasajonServices->getClientById($cliente);
        ...
        $transp = $this->nasajonServices->getClientByid($transp );
    ...
    public function createPloomesContactFromErpObject(object $contact, object $ploomesServices):string
    {
        ...
        $mailVendedor = (!empty($contact->codigoVendedor) ? $this->nasajonServices->getMailVendedorById($omie, $contact) : null);
        ...
        // ];
        // $cli = $this->nasajonServices->getClientById($contact);
        // $chavePix = [
    ...
    {
        return $this->nasajonServices->alteraClienteCRMToERP($contact);                            
                       
    }

    // createContactCrmToERP - cria um contato no CRM envia ao ERP
    public function createContactCrmToERP(object $contact):array
    {
        return $this->nasajonServices->criaClienteErp($contact);
    }
    ...
    /** 'Esqueletos' de requisições  **/
    //recebe os dados do cliente vindo do crm e monta o esqueleto da requisição de novo contato 
    public function createObjectErpClientFromCrmData($json, $ploomesServices):object
    {
        ...
        $contact->id = $decoded['New']['CNPJ'] ?? $decoded['New']['CPF'];
        $contact->codigo = $decoded['New']['Id'];
        $contact->qualificacao = ($decoded['New']['TypeId'] !== 1) ? "fisica":"juridica";
        $contact->nome = $decoded['New']['Name'];
        $contact->nome_fantasia = $decoded['New']['LegalName'] ?? null;
        $contact->inativo = ($prop['contact_CED4CBAD-92C7-4A51-9985-B9010D27E1A4'] === true) ? true : false;
        $contact->situacao_financeiro = null; //default: "nenhum" opções, adimplente ou inadimplente
        $contact->indicador_inscricao_estadual = null; //Default: "nao_contribuinte_icms"
        $contact->inscricao_estadual = $prop['contact_5D5A8D57-A98F-4857-9D11-FCB7397E53CB'] ?? null;
        $contact->contribuinte_icms = ($prop['contact_FC16AEA5-E4BF-44CE-83DA-7F33B7D56453']) ? true : false;
        $contact->email = $decoded['New']['Email'];
        $contact->observacao = $decoded['New']['Note'];
        $contact->grupo_empresarial = "gamatermic";
        $idErp = [];
        $idErp[] = $prop['contact_4F0C36B9-5990-42FB-AEBC-5DCFD7A837C3'] ?? null;
        $idErp[] = $prop['contact_6DB7009F-1E58-4871-B1E6-65534737C1D0'] ?? null;
        $idErp[] = $prop['contact_AE3D1F66-44A8-4F88-AAA5-F10F05E662C2'] ?? null;
        $idErp[] = $prop['contact_07784D81-18E1-42DC-9937-AB37434176FB'] ?? null;
        $contact->idErp = $idErp;
        ...
        $contact->tenant = $bases;

        $contatos = [];
        $contatos['nome'] = $prop['contact_3C521209-46BD-4EA5-9F41-34756621CCB4'] ?? null;
        $phones = [];
        ...
        $contatos['telefones'] = $phones ?? null;
        $contact->contatos[] = $contatos;
        $enderecos = [];
        $enderecos['tipo_logradouro'] = "lgr";
        $enderecos['logradouro'] = $decoded['New']['StreetAddress'];
        $enderecos['numero'] = $decoded['New']['StreetAddressNumber'];
        $enderecos['complemento'] = $decoded['New']['StreetAddressLine2'] ?? null;
        $enderecos['cep'] = $decoded['New']['ZipCode'] ?? null;
        $enderecos['bairro'] = $decoded['New']['Neighborhood'];
        $enderecos['tipo'] = 'local';
        $enderecos['uf_exterior'] = null;
        $enderecos['padrao'] = true;
        $enderecos['uf'] = $cliente['State']['Short'];
        $codSped = DiverseFunctions::getCountrySpedCode($cliente['Country']['Name']);
        $enderecos['pais'] = $codSped;
        $enderecos['ibge'] = $cliente['City']['IBGECode'];
        $enderecos['cidade'] = $cliente['City']['Name'];
        $contact->enderecos[] = $enderecos;
        //se quisermos buscar alguma informação do ERP temos o objeto $this->nasajonServices
